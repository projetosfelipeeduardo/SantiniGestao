...
class ReportController extends Controller
{
    ...
    public function gerarRelatorio(Request $request)
    {
        ...
        // Define as datas com base nos inputs ou usa o período padrão dos últimos 30 dias
        $fim = $request->filled('fim') ? date('Y-m-d', strtotime(str_replace('/', '-', $request->fim))) : date('Y-m-d');
        $inicio = $request->filled('inicio') ? date('Y-m-d', strtotime(str_replace('/', '-', $request->inicio))) : date('Y-m-d', strtotime('-30 days'));
